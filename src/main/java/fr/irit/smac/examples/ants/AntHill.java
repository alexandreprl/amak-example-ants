...
public class AntHill extends Amas<WorldEnvironment> {
	...
	public AntHill(WorldEnvironment environment, VectorialGraphicsPanel graphicsPanel) {
		...
		DrawableRectangle d = graphicsPanel.createRectangle(0, 0, 800, 600);
		d.setStrokeOnly();
		graphicsPanel.createRectangle(90, 20, 180, 40).setColor(new Color(0.9f, 0.9f, 0.9f, 0.8f)).setFixed().setLayer(5);

		graphicsPanel.createImage(20, 20, getClass().getResource("/ant.png").getFile()).setFixed().setLayer(10);
		antsCountLabel = (DrawableString) graphicsPanel.createString(45, 25, "Ants count").setFixed().setLayer(10);
